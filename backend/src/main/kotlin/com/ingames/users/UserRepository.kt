        ...
        if (DatabaseFactory.isConnectedToPostgres) {
            return DatabaseFactory.withConnection { conn ->
                ...
                if (rs.next()) {
                    ...
                    val name = rs.getString("name") ?: "Player"
                    val avatar = rs.getString("avatar") ?: "avatar_1"
                    val role = rs.getString("role") ?: "USER"
                    val isBlocked = rs.getBoolean("is_blocked")
                    return@withConnection UserProfile(
                        id = userId,
                        phone = phone,
                        name = name,
                        avatar = avatar,
            ...
            if (existing != null) {
                ...
                MemoryDataStore.wallets[newId] = mutableMapOf(
                    "deposit" to 80000L, // Initial demo balance ₹800.00
                    "winnings" to 45000L, // Initial demo balance ₹450.00
                    "bonus" to 0L,
                    "reserved" to 0L
                )
                return UserProfile(
            ...
            return DatabaseFactory.withConnection { conn ->
                ...
                if (rs.next()) {
                    UserProfile(
                        ...
                        name = rs.getString("name") ?: "Player",
                        avatar = rs.getString("avatar") ?: "avatar_1",
