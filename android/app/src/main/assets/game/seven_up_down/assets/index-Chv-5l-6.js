(function () {
  ...
  function statusText(value) {
    ...
    if (!element) {
      ...
      element.style.cssText =
        'position:fixed;left:50%;top:76px;transform:translateX(-50%);z-index:1000;' +
        'padding:6px 12px;border-radius:999px;background:rgba(20,8,48,.86);color:#fff;' +
        'font:600 11px Poppins,sans-serif;pointer-events:none;';
  ...
  function ensureDiceAnimationStyles() {
    ...
    style.textContent =
      '@keyframes sudDiceRoll{0%{transform:rotate(0deg) scale(1)}25%{transform:rotate(-18deg) scale(1.08)}50%{transform:rotate(22deg) scale(1.12)}75%{transform:rotate(-14deg) scale(1.06)}100%{transform:rotate(0deg) scale(1)}}' +
      '.sud-dice-rolling{animation:sudDiceRoll .18s linear infinite;will-change:transform}' +
      '.sud-dice-result{animation:sudDiceResult .35s cubic-bezier(.2,.8,.2,1)}' +
      '@keyframes sudDiceResult{0%{transform:scale(.72) rotate(-8deg);opacity:.55}65%{transform:scale(1.12) rotate(3deg);opacity:1}100%{transform:scale(1) rotate(0);opacity:1}}';
  ...
  function result(event) {
    ...
    window.__SUD_LAST = {
      roundId: event.roundId || round,
      dice1: first,
      dice2: second,
      diceSum: sum,
      winningBetType: event.winningBetType || type(sum),
    };
  ...
  async function bet(kind) {
    ...
    try {
      ...
      const body = await api('/api/games/7updown/bets', {
        ...
        body: JSON.stringify({
          roundId: round,
          bets: [{ betType: kind, stake: amount, idempotencyKey: id }],
        }),
      });
      ...
      window.parent?.postMessage?.({
        ...
        version: 1,
        type: 'WALLET_UPDATED',
        balance: body.data?.wallet?.totalBalance ?? body.data?.wallet?.cashBalance,
      }, '*');
    } catch (error) {
